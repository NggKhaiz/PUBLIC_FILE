...
# │                                                                 │
# │   GitHub: https://github.com/https://github.com/NggKhaiz/Raven_Scanner                   │
# │                                                                 │
# └─────────────────────────────────────────────────────────────────┘
#
# Usage:
...
XRAY_HOME = os.path.join(os.path.expanduser("~"), ".Raven_Scanner")
...
def xray_find_binary(custom_path: Optional[str] = None) -> Optional[str]:
    """Find xray binary. Search order: custom_path > PATH > ~/.Raven_Scanner/bin/xray."""
...
def xray_install() -> Optional[str]:
    """Download xray-core to ~/.Raven_Scanner/bin/. Returns binary path or None."""
...
def _help_show_page(title: str, content: List[str]):
    ...
    while True:
        ...
        t = f" {A.BOLD}{A.WHT}Raven_Scanner{A.RST} {A.DIM}v{VERSION}{A.RST}"
...
def _help_getting_started() -> List[str]:
    return [
        "",
        f" {A.BOLD}{A.CYN}What is Raven_Scanner?{A.RST}",
        ...
        f"   {A.WHT}2.{A.RST} Raven_Scanner resolves domains to Cloudflare edge IPs",
...
def _help_scan_modes() -> List[str]:
    return [
        ...
        f"   Place config files in the directory where you run Raven_Scanner.",
        ...
        f"   3. Raven_Scanner fetches and parses the configs automatically",
        "",
        ...
        f"   4. Raven_Scanner generates a config for each IP and scans all",
        "",
...
def _help_xray_test() -> List[str]:
    return [
        ...
        f"   Let Raven_Scanner test all presets to find the best one for you.",
        "",
        f" {A.BOLD}{A.CYN}Xray binary{A.RST}",
        f"   Xray-core is auto-installed to {A.WHT}~/.Raven_Scanner/bin/xray{A.RST}",
...
def _help_clean_finder() -> List[str]:
    return [
        ...
        "",
        f"   3. Raven_Scanner tests TCP+TLS connectivity to each IP",
        ...
        f"   By default Raven_Scanner scans all official Cloudflare ranges.",
        ...
        f"   Raven_Scanner verifies each IP actually serves Cloudflare by",
...
def _help_deploy() -> List[str]:
    return [
        ...
        f"   5. Raven_Scanner installs Xray, generates UUID + keys",
...
def _help_worker_proxy() -> List[str]:
    return [
        ...
        f"   3. Raven_Scanner generates a Worker script (JavaScript)",
        ...
        f"   6. Raven_Scanner builds a new URI with the Worker as address/SNI",
...
def _help_cli_reference() -> List[str]:
    return [
        ...
        f"   {A.WHT}--uninstall{A.RST}          Remove everything Raven_Scanner installed",
        "",
...
def tui_show_guide():
    ...
    while True:
        ...
        bx(f" {A.DIM}https://github.com/https://github.com/NggKhaiz/Raven_Scanner{A.RST}")
        out.append(f"{A.CYN}{'=' * (W + 2)}{A.RST}")
...
def tui_pick_file() -> Optional[Tuple[str, str]]:
    ...
    while True:
        ...
        out.append(f"{A.CYN}╔{'═' * W}╗{A.RST}")
        title = f" ⚡ {A.BOLD}{A.WHT}Raven_Scanner{A.RST} {A.DIM}v{VERSION}{A.RST}"
...
async def _tui_run_pipeline(args, cli_uri: str = ""):
    ...
    if not xray_bin:
        _w(f" {A.YEL}Xray not found. Installing to ~/.Raven_Scanner/bin/...{A.RST}\n")
        _fl()
...
def deploy_fresh_config(
    ...
    parsed = {
        ...
        "port": port,
        "name": f"Raven_Scanner-{protocol}-{transport}",
        "type": transport,
        "security": security,
        "sni": sni,
        "host": sni,
...
def generate_configless_base(
    ...
    if protocol == "vmess":
        vless_parsed = {
            ...
            "name": "Raven_Scanner-vless-ws", "type": "ws",
        ...
        vless_uri = build_vless_uri(vless_parsed, default_sni, "Raven_Scanner-vless-ws")
...
def deploy_run_pipeline(ds: "DeployState", print_fn) -> bool:
    ...
    try:
        for i, parsed in enumerate(ds.parsed_configs):
            tag = f"Raven_Scanner-{parsed.get('protocol', 'vless')}-{i + 1}"
...
def _cm_build_client_uri(inbound: dict, uuid_val: str, server_ip: str) -> Optional[str]:
    ...
    try:
        ...
        if security == "reality":
            ...
            parsed["sni"] = sni
        tag = f"Raven_Scanner-{protocol}-{port}"
...
def _uninstall_all() -> Tuple[bool, str]:
    """Remove everything Raven_Scanner installed on this system."""
    _out: list = []
    _had_errors = False
    ...
    if sys.platform in ("win32", "darwin"):
        if os.path.isdir(XRAY_HOME):
            ...
            _log("Nothing to remove (no local Raven_Scanner directory)")
    ...
    # --- 4. Remove local client dir (~/.Raven_Scanner/) ---
    if os.path.isdir(XRAY_HOME):
        shutil.rmtree(XRAY_HOME, ignore_errors=True)
        if os.path.isdir(XRAY_HOME):
            _log_err(f"Could not fully remove {XRAY_HOME}")
    ...
    while True:
        ...
        if key == "a":
            ...
            if _add_ok:
                ...
                _uri_tag = f"Raven_Scanner-{protocol}-{new_port}"
                try:
...
def _worker_proxy_generate_script(origin_host: str, origin_port: int,
    ...
    return f"""\
// Raven_Scanner Worker Proxy — route ANY SNI to origin
...
class Dashboard:
    ...
    def draw_help_popup(self):
        ...
        lines.append(f"  {A.DIM}  https://github.com/https://github.com/NggKhaiz/Raven_Scanner{A.RST}")
        lines.append(f"  {A.CYN}{'=' * W}{A.RST}")
...
""",
    ...
    p.add_argument("--xray-install", action="store_true",
                   help="Download and install xray-core to ~/.Raven_Scanner/bin/")
    ...
    p.add_argument("--uninstall", action="store_true",
                   help="Remove everything Raven_Scanner installed")
    args = p.parse_args()
